//
//  PastFlight+CoreDataClass.swift
//  Trackify
//
...
public class PastFlight: NSManagedObject {
        ...
        if let users = try? context.fetch(NSFetchRequest(entityName: "SavedUser")) {
            if !users.isEmpty {
                ...
                for userFlight in signedInUser.pastFlights! {
                    let pastFlight = userFlight as! PastFlight
                ...
                if let flight = NSEntityDescription.insertNewObject(forEntityName: "PastFlight", into: context) as? PastFlight {
